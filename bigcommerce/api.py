...
class Connection(object):
    ...
    def request_json(self, method, path, data=None, is_xml=False):
        response, content = self.request(method, path, data, is_xml)
    ...
    def request(self, method, path, body=None, is_xml=False):
        http = httplib2.Http()
    ...
    def update_order_status(self, order, status):
        return self.request_json('PUT','/orders/%s' % order, '<order><status_id>%s</status_id></order>' % status, True)
...
class Order(Resource):
    ...
    def products(self):
        """ Gets the products for this order"""
        return [Product(product) for product in self.client.request('GET','/orders/%s/products' % str(self.id))]

    def update_status(self, status):
        """Updates the order status"""
        response, content = self.client.request_json('PUT','/orders/%s' % str(self.id), '<order><status_id>%s</status_id></order>' % str(status), True)
...
class OrderStatuses(Resource):
    ...
    def get(self):
        ...
        categories = self.client.request_json('GET', '/orderstatuses')
        return [Category(category) for category in categories]

    @classmethod
    def get_by_id(self, id):
        """Returns an individual category by given ID"""
        category = self.client.request_json('GET', '/orderstatuses/' + str(id))
        return Category(category)
